# -*- coding: utf-8 -*-
"""
analyze_cre_sem_level.py

Purpose
-------
Analyze CRE-Sem-Level raw VLM scores (semantic features injected as levels) vs human reference.

Example
-------
python src/analyze_cre_sem_level.py \
  --llm sample_data/sample_cre_sem_level_scores_raw.xlsx \
  --human sample_data/sample_human_ratings.xlsx \
  --out sample_outputs/sample_cre_sem_level_reliability_summary.xlsx
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Analyze CRE-Sem-Level raw VLM scores (semantic features injected as levels) vs human reference.")
    parser.add_argument("--base-dir", type=str, default=None)
    parser.add_argument("--llm", type=str, default="sample_data/sample_cre_sem_level_scores_raw.xlsx")
    ...
    parser.add_argument("--out", type=str, default="sample_outputs/sample_cre_sem_level_reliability_summary.xlsx")
    ...
    with pd.ExcelWriter(out_path) as writer:
        df_by_run.to_excel(writer, sheet_name="semlvl_by_run", index=False)
        df_all_runs.to_excel(writer, sheet_name="semlvl_all_runs", index=False)
        df_stab_per_image.to_excel(writer, sheet_name="semlvl_stab_img", index=False)
        df_stab_summary.to_excel(writer, sheet_name="semlvl_stab_sum", index=False)
