...
Purpose
-------
Analyze SRE (rubric-constrained evaluation) raw VLM scores against human reference scores.

Outputs
-------
An Excel summary with:
- correlations and leniency by run_id
- correlations and leniency across all runs
- per-image stability metrics (SD and CV) across runs
- stability summary per dimension

Example
-------
python src/analyze_sre.py \
  --llm sample_data/sample_sre_scores_raw.xlsx \
  --human sample_data/sample_human_ratings.xlsx \
  --out sample_outputs/sample_sre_reliability_summary.xlsx
"""
...
def compute_leniency(h_scores: np.ndarray, e_scores: np.ndarray, max_score: float = 10.0) -> float:
    """Leniency = mean(E_hat - H_hat), normalized to [0,1] by max_score."""
...
def load_human_scores(primary: Path, fallback: Path | None, id_col: str | None) -> tuple[pd.DataFrame, str, Path]:
    """Load human score table. Uses fallback if the primary does not exist."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Analyze SRE raw VLM scores vs human reference.")
    parser.add_argument("--base-dir", type=str, default=None, help="Project root (optional).")
    parser.add_argument("--llm", type=str, default="sample_data/sample_sre_scores_raw.xlsx", help="LLM raw SRE results.")
    parser.add_argument("--human", type=str, default="sample_data/sample_human_ratings.xlsx", help="Human scores (primary).")
    parser.add_argument("--human-fallback", type=str, default="sample_data/sample_human_ratings_fallback.xlsx", help="Human scores (fallback).")
    parser.add_argument("--out", type=str, default="sample_outputs/sample_sre_reliability_summary.xlsx", help="Output Excel path.")
    parser.add_argument("--id-col", type=str, default=None, help="Override ID column name (optional).")
    parser.add_argument("--max-score", type=float, default=10.0, help="Max score used for normalization.")
    ...
    print(f"Loaded LLM raw results: {llm_path} (rows={len(df_llm)})")
    ...
    with pd.ExcelWriter(out_path) as writer:
        df_by_run.to_excel(writer, sheet_name="cre_vs_human_by_run", index=False)
        df_all_runs.to_excel(writer, sheet_name="cre_vs_human_all_runs", index=False)
        df_stab_per_image.to_excel(writer, sheet_name="cre_stability_per_image", index=False)
        df_stab_summary.to_excel(writer, sheet_name="cre_stability_summary", index=False)
